...
def EM_step(pi, s, t, sentence_pairs, f_vocab, e_vocab):
    ...
    for f_len in set(f_lengths):
        ...
        for d in xrange(-f_len + 1, f_len):
            entry = np.log2(s2[f_len][d]) - np.log2(ctr[f_len])
...
def EM(pi, s, t, sentence_pairs, err_thresh = .00001, max_iterations = 50, verbose = True):
...
def most_probable_alignment(pi, s, t, sentence_pair):
