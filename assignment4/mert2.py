...
def rerank(weights = default_weights):
  ...
  for s in xrange(0, num_sents):
    ...
    for (num, hyp, feats) in hyps_for_one_sent:
      ...
      words = hyp.split(' ')
      score += weights['length'] * len(words)
      score += weights['untranslated'] * [is_ascii(word) for word in words].count(False)
      if score > best_score:
    ...
    for param in weights.keys():
      ...
      for s in xrange(num_sents):
        ...
        for (num, hyp, feats_str) in hyps_for_one_sent:
          ...
          feats['untranslated'] = [is_ascii(word) for word in words].count(False)
          slope = feats[param]
          intercept = 0.0
...
best_weights = {'length': 1.2216367107749555,
 'p(e)': -0.9961404011464516,
 'p(e|f)': -0.7943146359706527,
 'p_lex(f|e)': -0.6507362196174753,
 'untranslated': -12.690339999999999}
